...
def compute_superpoint_keypoints(img, nms_dist=4, conf_thresh = 0.015, nn_thresh =0.7, cuda = False):
    fe = SuperPointFrontend(weights_path='models/superpoint_v1.pth',
                          nms_dist= nms_dist,
                          conf_thresh= conf_thresh,
                          nn_thresh= nn_thresh,
                          cuda= cuda )
    """Compute Superpoint Keypoint for an individual image, reformating it as (x,y, scale, orientation) - descriptor
...
def superpoint_tangent_images(tex_image, base_order, sample_order, image_shape, opt = 'superpoint', crop_degree=0):
    """
    ...
    for i in range(tex_image.shape[1]):
        print(i)
        gray_img = process_img(tex_image[:, i, ...])
        kp_details = compute_superpoint_keypoints(gray_img)
...
def superpoint_equirectangular(img, crop_degree=0):
    """
    ...
    # ----------------------------------------------
    erp_kp_details = compute_superpoint_keypoints(img)
...
def main():
    ...
    img = load_torch_img('Image/7.png')[:3, ...].float()
    ...
    #compute_superpoint_keypoints(gray_img)

    # Keypoint in tangen planes

    tangent_image_kp, tangent_image_desc = superpoint_tangent_images(tex_image, base_order, sample_order, img.shape[-2:], crop_degree=0)

    #print('Num Tangent Image Keypoints:', tangent_image_kp.shape[0])



    print(torch.max(tangent_image_kp,axis = 0))
    #print(tangent_image_desc.shape)

    # Keypoints on the Equirectangular Image
    erp_kp_details = superpoint_equirectangular(process_img(img), crop_degree=0)
    erp_kp = erp_kp_details[0]
    erp_desc = erp_kp_details[1]
    ...
    #ax.plot(erp_kp[:, 0], erp_kp[:, 1], 'r.', markersize=2.5)
    #tangent_image_kp[(tangent_image_kp[:, [0]] >= img.shape[-2] - 1
    #              ).expand_as(tangent_image_kp)] = float('nan')
    ax.plot(tangent_image_kp[:, 0], tangent_image_kp[:, 1], 'b.', markersize=2.5)
    plt.axis('off')
