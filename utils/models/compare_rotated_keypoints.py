...
import magic_sp.superpoint as magic_sp
import train_sp.superpoint as train_sp
...
def computes_superpoint_keypoints(img, opt, nms_dist=4, conf_thresh = 0.015, nn_thresh =0.7, cuda = False):
    ...
    #if opt.enswith('.pth'):
    #fe = magic_sp.SuperPointFrontend(weights_path = 'superpoint_v1.pth', nms_dist= nms_dist, conf_thresh= conf_thresh, nn_thresh= nn_thresh,cuda= cuda )

    #else:

    fe = train_sp.SuperPointFrontend(weights_path = 'superpoint-trained.pth.tar', nms_dist= nms_dist, conf_thresh = conf_thresh, nn_thresh= nn_thresh, cuda = cuda )
...
def process_image_to_keypoints(image_path, corners, scale_factor, base_order, sample_order, opt, mode):
    ...
    return img, tangent_image_kp, np.transpose(tangent_image_desc,[1,0])

def main():

    parser = argparse.ArgumentParser(description = 'Tangent Plane')
    parser.add_argument('--opt', default ='superpoint')
    parser.add_argument('--mode', default = 'tangent')
    ...
    img1, pts1, desc1 = process_image_to_keypoints(args.path_o, corners, scale_factor, base_order, sample_order, args.opt, args.mode)

    img2, pts2, desc2 = process_image_to_keypoints(args.path_r, corners, scale_factor, base_order, sample_order, args.opt, args.mode)

    #print(pts1.shape)
    #print(pts2.shape)


    M = nn_match_two_way(desc1, desc2)
    ...
    ax1.plot(pts1[:, 0], pts1[:, 1], 'r.', markersize=1.5)
    ...
    ax2.plot(pts2[:, 0], pts2[:, 1], 'r.', markersize=1.5)
